/*This class implements an indexable min heap
...
public class MinHeap {
    ...
    //create heap object, initialize the array size, set up hashmaps
    ...
    public static void addToHeap(Apartment apartment){
        ...
        if ((addressMap.get(apartment.getAddress()) == null || addressMap.get(apartment.getAddress()).get(apartment.getApartmentNumber()) == null
            ...
            addressMap.put(apartment.getAddress(), new HashMap(){{put(apartment.getApartmentNumber(), new HashMap(){{put(apartment.getZipCode(), finalCurrentNode);}});}});
        }
        ...
        if (itemsInHeap == 0){
            ...
            addressMap.put(apartment.getAddress(), new HashMap(){{put(apartment.getApartmentNumber(), new HashMap(){{put(apartment.getZipCode(), 0);}});}});
        ...
        //if there are items
            ...
            //check the heap property for the MIN HEAP
            ...
            while ((heap[currentNode].compareToRent(heap[parentNode])) > 0){
                ...
                addressMap.put(apartment.getAddress(), new HashMap(){{put(apartment.getApartmentNumber(), new HashMap(){{put(apartment.getZipCode(), finalParentNode);}});}});
                int finalCurrentNode1 = currentNode;
                addressMap.put(tempApartment.getAddress(), new HashMap(){{put(tempApartment.getApartmentNumber(), new HashMap(){{put(tempApartment.getZipCode(), finalCurrentNode1);}});}});
        ...
        //Now need to update the indirection for the min rent by city
        ...
        if(indirection.containsKey(city)){
            //compare the new apartment rent value with the value already in the hash map to see if it is lower
            if(apartment.getRent() < heap[indirection.get(city)].getRent()){
                //if it is lower, update the index
                indirection.replace(city, currentNode);
            }
            //if it is not less, then do not update the hashmap with an index
        }
    ...
    //remove an apartment from the heap
    public static Apartment removeFromHeap(int startingLocation){
        ...
        while(heap[(2*currentNode) + 1] != null || heap[(2*currentNode) + 2] != null) {
            ...
            leftChildDifference = heap[(2 * currentNode) + 1].compareToRent(heap[(currentNode)]);
            if (heap[(2 * currentNode) + 2] != null) {
                rightChildDifference = heap[(2 * currentNode) + 2].compareToRent(heap[(currentNode)]);
            }
            else {rightChildDifference = 0;}

            //if the difference is greater than 0, the parent is more than the child
        ...
        //return the removed apartment
        return apartmentToRemove;
    }

    //retrieve the lowest rent apartment and return it, do not delete!
    public static Apartment retrieveLowest(){
        return heap[0];
    }

    //retrieve the lowest rent apartment BY CITY and return it, do not delete!
    public static Apartment retrieveLowestByCity(String city){
    ...
    public static Apartment findInHeap(String address, String aptNum, int zipCode, double newRent){
        ...
        reheap(addressMap.get(address).get(aptNum).get(zipCode), heap[addressMap.get(address).get(aptNum).get(zipCode)], newRent);
        return heap[addressMap.get(address).get(aptNum).get(zipCode)];
    }

    public static void reheap(int currentNode, Apartment apartmentToReheap, double newRent){
        //Set the new rent
        apartmentToReheap.setRent(newRent);

        boolean swapUp = false;

        //check the heap property for the MIN HEAP
        //if a positive value is returned, the parent is greater than the child
        //So, while the value is less than 0, we want to swap the parent and child
        int parentNode = (int) Math.floor((currentNode - 1)/2);
        while ((heap[currentNode].compareToRent(heap[parentNode])) > 0){
            swapUp = true;
            ...
            //Move the child up
            heap[parentNode] = apartmentToReheap;
            //need to update indirection
            ...
            //update the index in the address hash map if we need to update the locations in the array for the
            int finalParentNode = parentNode;
            addressMap.put(apartmentToReheap.getAddress(), new HashMap(){{put(apartmentToReheap.getApartmentNumber(), new HashMap(){{put(apartmentToReheap.getZipCode(), finalParentNode);}});}});
            int finalCurrentNode1 = currentNode;
            addressMap.put(tempApartment.getAddress(), new HashMap(){{put(tempApartment.getApartmentNumber(), new HashMap(){{put(tempApartment.getZipCode(), finalCurrentNode1);}});}});
        ...
        if (!swapUp){
            ...
            while(heap[(2*currentNode) + 1] != null || heap[(2*currentNode) + 2] != null) {
                ...
                if (leftChild && !rightChild) {
                    ...
                    //update the index in the address hash map if we need to update the locations in the array for the
                    int finalLeftChild = leftIndex;
                    addressMap.put(apartmentToReheap.getAddress(), new HashMap(){{put(apartmentToReheap.getApartmentNumber(), new HashMap(){{put(apartmentToReheap.getZipCode(), finalLeftChild);}});}});
                    int finalCurrentNode1 = currentNode;
                    addressMap.put(temp.getAddress(), new HashMap(){{put(temp.getApartmentNumber(), new HashMap(){{put(temp.getZipCode(), finalCurrentNode1);}});}});
                ...
                //if only the rightChild needs to be swapped
                    ...
                    //update the index in the address hash map if we need to update the locations in the array for the
                    int finalRightChild = rightIndex;
                    addressMap.put(apartmentToReheap.getAddress(), new HashMap(){{put(apartmentToReheap.getApartmentNumber(), new HashMap(){{put(apartmentToReheap.getZipCode(), finalRightChild);}});}});
                    int finalCurrentNode1 = currentNode;
                    addressMap.put(temp.getAddress(), new HashMap(){{put(temp.getApartmentNumber(), new HashMap(){{put(temp.getZipCode(), finalCurrentNode1);}});}});
